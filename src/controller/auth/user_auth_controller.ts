...
export const signUp = async (req: Request, res: Response) => {
    try {
        const {
            firstName, lastName, email, password, role, address, city, location, state, country, zip, street,
            preferredLocations, services, availability, availabilityTime, preferredJobType
        } = req.body;

        // Validate mandatory fields
        if (!firstName || !lastName || !email || !password || !role || !address || !city || !state || !country) {
            return res.status(400).json({
                status: 400,

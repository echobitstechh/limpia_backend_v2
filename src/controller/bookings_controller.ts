...
export const getNearByBookings = async (req: Request, res: Response) => {
    try {
        ...
        const { preferredLocations, services, availability, availabilityTime } = cleaner;
        ...
        const filteredBookings = bookings.filter((booking) => {
            ...
            matchesAvailability = isWithinAvailability(booking.cleaningTime, availability);
            ...
            if (availabilityTime) {
                ...
                console.log(`Booking ${booking.id}: No availability periods specified.`);
            }

            const matches = matchesLocation && matchesAvailability && matchesPeriod;

            if (!matches) {
                console.log(`Booking ${booking.id} skipped: One or more conditions did not match.`);
            }
            ...
            .map((b) =>
                b.property?.address
                    ? `${b.property.address.street || ''}, ${b.property.address.city}, 
                       ${b.property.address.state}, ${b.property.address.country}`
                    : null
            )
            .filter(Boolean)
            .join('|');
        ...
        const sortedBookings = bookingsWithDistance.sort((a, b) => a.distance - b.distance);
